...
class AttentionWrapper(nn.Module):
    ...
    def __init__(
        ...
        self.pre_attention_kv_norm = nn.LayerNorm(embed_dim) if is_cross_attention else None
        ...
        self.ff = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * ff_dim),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        key = self.pre_attention_kv_norm(key) if key is not None and self.pre_attention_kv_norm is not None else key
        value = self.pre_attention_kv_norm(value) if value is not None and self.pre_attention_kv_norm is not None else value
    
        # mask is only useful for cross attention, ignore attention weights
        attn_output, *_ = self.attention(query, key, value, key_padding_mask=mask) if self.is_cross_attention else self.attention(query, query, query)
...
class X(nn.Module):
    ...
    def forward(
        ...
        latent = repeat(rearrange(self.latent, "N D -> 1 N D"), "1 N D -> B N D", B = batch_size)
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_lambda = x.new_ones((batch_size,)) if i == len(self.layers) - 1 else self.lambda_layer(torch.mean(latent, dim=1))
            # calculate current prediction from current layer
            layer_predictions = self.output_layer(
                torch.mean(latent, dim=1)
            )
            ...
            halt = torch.bernoulli(layer_lambda.view(-1)) * (1 - halted)
            ...
            y_m = y_m * repeat(1 - halt, "B -> B C", C=self.num_classes) + layer_predictions * repeat(halt, "B -> B C", C=self.num_classes)
...
class XLoss(nn.Module):
    ...
    def forward(self, probas, preds, labels):
        ...
        return self.reconstruction_loss(probas, preds, labels) + self.regularization_loss(probas)
...
if __name__ == "__main__":
    ...
    loss = XLoss(nn.CrossEntropyLoss(reduction='mean'), lambda_p=0.9, max_layers=12)

    # help(summary)
    summary(model, input_size=(seq_length, embed_dim), batch_size=10, device='cpu')

    x = torch.randn(10, seq_length, embed_dim)
    mask = torch.rand(10, seq_length)
    mask = mask <= 0.5
    
    probas, preds, final_proba, final_pred = model(
        x,
        mask
    )
    
    l = loss(probas, preds, torch.ones((10,)).long())
    print(l)
