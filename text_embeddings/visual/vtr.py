...
def text2image(text: str, font: str, font_size: int = 14) -> Image:
    ...
    line_width, line_height = image_font.getsize(text)

    img = Image.new("L", (line_width, line_height))
...
class VTRTokenizer(PreTrainedTokenizerBase):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.special_tokens is None:
            self.special_tokens = {
                "CLS": self.text2blocks("[CLS]", stride=stride),
                "SEP": self.text2blocks("[SEP]", stride=stride),
            }
        ...
        for first_text, second_text in zip_longest(text, text_pair):
            ...
            first_embeddings = self.text2blocks(first_text, stride=stride)
            second_embeddings = self.text2blocks(second_text, stride=stride)
    ...
    def text2blocks(self, text: str, stride: int) -> np.ndarray:
        """Convert text into an numpy array, in (sequence_length, height, width) shape.
        ...
        text : str
            Input text
        stride : int
            Stride for image block generation
        ...
        image = text2image(text, font=self.font, font_size=self.height)
        image_array = np.asarray(image)

        return np.squeeze(
            sliding_window_view(image_array, (image_array.shape[0], self.width)),
            axis=0,
        )[::stride]
    ...
    def build_inputs_with_special_tokens(
        ...
        if token_ids_1 is None:
            return token_ids_0
        height = max(
            self.special_tokens["CLS"].shape[1], 
            token_ids_0.shape[1],
            self.special_tokens["SEP"].shape[1], 
            token_ids_1.shape[1],
            self.special_tokens["SEP"].shape[1]
        )
        ...
        return np.concatenate(
            [
                np.pad(self.special_tokens["CLS"], ((0, 0), (0, height-self.special_tokens["CLS"].shape[1]), (0, 0)), 'constant', constant_values=0),
                np.pad(token_ids_0, ((0, 0), (0, height - token_ids_0.shape[1]), (0, 0)), 'constant', constant_values=0),
                np.pad(self.special_tokens["SEP"], ((0, 0), (0, height - self.special_tokens["SEP"].shape[1]), (0, 0)), 'constant', constant_values=0),
                np.pad(token_ids_1, ((0, 0), (0, height - token_ids_1.shape[1]), (0, 0)), 'constant', constant_values=0),
                np.pad(self.special_tokens["SEP"], ((0, 0), (0, height - self.special_tokens["SEP"].shape[1]), (0, 0)), 'constant', constant_values=0)
            ],
            axis=0
        )
    ...
    def _pad(
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                ...
                encoded_inputs[self.model_input_names[0]] = required_input + [np.zeros((len(required_input[0]), self.width))] * difference
                ...
                encoded_inputs[self.model_input_names[0]] = [np.zeros((len(required_input[0]), self.width))] * difference + required_input
            else:
